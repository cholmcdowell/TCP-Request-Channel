...
//     nchan = new TCPRequestChannel(chname, TCPRequestChannel::CLIENT_SIDE);
//     return nchan;
// }
...
void file_thread_function (string fname, TCPRequestChannel* chan, int b,
BoundedBuffer* rb) {
...
void worker_thread_function (TCPRequestChannel* chan, BoundedBuffer* rb,
BoundedBuffer* sb, int b) {
    char buf[b];
    double result = 0.0;
    char recvbuf[b];
    while (true) {
...
int main (int argc, char *argv[]) {
    ...
    string a = ""; // ip address
    string r = ""; // port number the server is deploying its services to
    // read arguments
    int opt;
    ...
    if (!filereq) {
        for (int i = 0; i < p; i++) {
            patients[i] = thread(patient_thread_function, n, (i+1),
            &request_buffer);
        }
    }
    else {
    ...
    for (int i = 0; i < w; i++) {
        workers[i] = thread(worker_thread_function, wchans.at(i), &request_buffer,
        &response_buffer, m);
    }
    thread* hists = new thread[h];
    for (int i = 0; i < h; i++) {
    ...
    int secs = ((1e6*end.tv_sec - 1e6*start.tv_sec) + (end.tv_usec -
    start.tv_usec)) / ((int) 1e6);
    int usecs = (int) ((1e6*end.tv_sec - 1e6*start.tv_sec) + (end.tv_usec -
    start.tv_usec)) % ((int) 1e6);
    cout << "Took " << secs << " seconds and " << usecs << " micro seconds" <<
    endl;
