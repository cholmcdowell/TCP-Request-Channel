...
int main (int argc, char* argv[]) {
	...
	while (true) {
		...
		int a_sockfd = control_channel ->accept_conn();
		// create a new channel w/ accepted socket (the socket is already established)
		TCPRequestChannel* new_chan = new TCPRequestChannel(a_sockfd);
		// dispatch a new thread to handle_process_loop
		thread n_thread(handle_process_loop, new_chan);
		n_thread.detach();
		// since main thread is not blocked, it can accept more connections (use detach instead of join)
	}
